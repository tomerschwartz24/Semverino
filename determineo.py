...
def check_if_tag_exist():
    try:
        ...
        logging.warning(' It seems like the latest commit does not have any tags!')
        if no_tag_found:
            return None
    if HEAD_TAG is not None:
        logging.info(' Latest commit is tagged as: {}'.format(HEAD_TAG))
        return HEAD_TAG
...
def check_if_previous_commit_is_tagged():
    try:
        ...
        logging.warning(' It seems like the previous commit does not have any tags!')
